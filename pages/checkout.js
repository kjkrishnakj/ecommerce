...
const Checkout = ({
  ...
  const vemail = () => {
    if (
      ...
      toast.error("Enter a valid email address.", { autoClose: 1000 });
      setEmail("");
    }
  };
  ...
  const vpincode = () => {
    if (!pincode || pincode.length !== 6 || !pincode.match(/^\d+$/)) {
      toast.error("Enter a valid 6-digit pincode.", { autoClose: 1000 });
      setPincode("");
    }
  };

  const vphone = () => {
    if (!phone || !phone.match(/^\d+$/) || phone.length !== 10) {
      toast.error("Enter a valid 10-digit phone number.", { autoClose: 1000 });
      setPhone("");
    }
  };
      ...
      if (e.target.value.length === 6) {
        try {
          ...
          if (response.ok) {
            ...
            toast.error("Either pincode is invalid or not serviceable!🥲", {
              autoClose: 1000,
            });
            setState("");
            setCity("");
          ...
          toast.error(
            "Error fetching pincode details. Please try again later!",
            { autoClose: 1000 }
          );
          setState("");
          setCity("");
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <div className="flex flex-col mb-10 items-center border-b bg-white py-4 sm:flex-row sm:px-10 lg:px-20 xl:px-32"></div>
